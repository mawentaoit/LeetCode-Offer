 ...
 */
TreeNode::ptr createBinaryTree(int tall, int min = 1, int max = 100) {
        if(tall <= 0) return nullptr;
        TreeNode::ptr p(new TreeNode());
...
void printInOrder(TreeNode::ptr head, int height, string to, int len) {
 ...
 * 中序打印二叉树
 */
void printTree(TreeNode::ptr head) {
